...
const ProgressTracker = () => {
  ...
  const weeklyGoal = 20;
  const weeklyProgress = Math.min(Math.round((stats.workoutsCompleted / weeklyGoal) * 100), 100);
      ...
      <div className="grid grid-cols-1 gap-4 mb-6">
        ...
        <div className="relative overflow-hidden glass-panel p-4 rounded-xl border border-primary/20 bg-surface-secondary/50">
          ...
          <div className="relative flex items-center justify-between mb-2">
            <span className="text-xs uppercase tracking-wide text-text-tertiary">Workouts</span>
          </div>
          <div className="relative flex items-baseline gap-2">
            <Trophy className="text-primary" size={20} />
            <span className="text-2xl font-bold text-text-primary">{stats.workoutsCompleted}</span>
        ...
        <div className="relative overflow-hidden glass-panel p-4 rounded-xl border border-accent/20 bg-surface-secondary/50">
          ...
          <div className="relative flex items-center justify-between mb-2">
            <span className="text-xs uppercase tracking-wide text-text-tertiary">Calories</span>
          </div>
          <div className="relative flex items-baseline gap-2">
            <Flame className="text-accent" size={20} />
            <span className="text-2xl font-bold text-text-primary">{stats.caloriesBurned.toLocaleString()}</span>
        ...
        <div className="relative overflow-hidden glass-panel p-4 rounded-xl border border-secondary/20 bg-surface-secondary/50">
          ...
          <div className="relative flex items-center justify-between mb-2">
            <span className="text-xs uppercase tracking-wide text-text-tertiary">Active Time</span>
          </div>
          <div className="relative flex items-baseline gap-2">
            <Activity className="text-secondary" size={20} />
            <span className="text-2xl font-bold text-text-primary">{(stats.activeMinutes / 60).toFixed(1)}</span>

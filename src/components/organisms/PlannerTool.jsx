  ...
  return (
    <div className="glass-panel rounded-2xl p-6 border border-surface-border">
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="text-sm text-primary hover:text-primary/80 transition-colors">Edit Plan</button>
      </div>

      <div className="space-y-6">
        {weeklyPlan.map((day, dayIdx) => (
          <div key={dayIdx} className="relative pl-6 border-l-2 border-surface-border hover:border-primary transition-colors group">
            ...
            <div className="flex justify-between items-baseline mb-3">
              ...
              <span className="text-xs text-text-tertiary uppercase tracking-widest">{day.focus}</span>
            </div>
            {day.exercises.length > 0 ? (
              <div className="space-y-2">
                ...
                <div className="text-sm text-text-tertiary italic">Rest Day or No Workout Planned</div>
            )}
          </div>
        ))}
      </div>

...
export const ServiceCard = ({ service }) => {
  const { icon: Icon, title, description, features, price, priceDetail, popular, badge } = service;
  ...
  return (
    ...
    >
      {/* Popular Badge */}
      {badge && (
        <div className={`absolute -top-3 right-6 px-6 py-2 text-white text-xs font-bold rounded-full flex items-center gap-1 ${
          popular ? 'bg-linear-to-r from-primary to-secondary' : 'bg-surface-secondary border border-surface-border text-text-primary'
        }`}>
      ...
      <p className="font-body text-text-tertiary mb-6 leading-relaxed">{description}</p>

      {/* Price */}
      <div className="mb-6">
        <span className="text-4xl font-bold text-text-primary">{price}</span>
        {priceDetail && <span className="text-text-tertiary text-lg">{priceDetail}</span>}
      </div>
